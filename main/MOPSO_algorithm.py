...
for run in range(1, max_run+1):
    ...
    while it < iter_max and nochange_flag < nochange_max:
        ...
        # 更新archive解集
        print('第', it, '次迭代  ', '惯性权重：', omiga, 'archive解集：', len(archive_adapt), g_best_adapt)
        # 对新生成的解进行选择，保留前沿解
    ...
    file.to_csv(os.path.join(result_dir, MOPSO_dir, 'edge_result_' + str(run) + '.csv'), index=False, header=False)
    t4 = time.time()
    run_time.append(t4 - t3)
    print('Configuration optimizing by MOPSO is finished in ' + str(t4 - t3) + 's.')
pd.DataFrame(run_time).to_csv(os.path.join(result_dir, 'time', 'MOPSO.csv'), index=False, header=False)
