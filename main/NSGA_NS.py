...
def choose(initials, refer_point, adaption, num_reserve):
    ...
    while len(choose_generate) < num_reserve:
        ...
        if len(choose_generate) + len(inx) <= num_reserve:
            ...
            i_index = last_selection(choose_adap, adap[inx], num_reserve - len(choose_generate), refer_point,
                                     nomal_point)
...
def nomalization(S1):
    ...
    for j in range(len(S1[0][-1])):
        ...
        extreme_points.append(
            S1[A.index(min(A))])  # 每个维度方向上的极端点即为在其他维度方向上的值均很小的解（由于非支配关系的原因，其他维度方向上的值很小时，当前维度上的值必定相对较大）
    ...
    while k == 1:
        try:
            ...
            for s in range(len(B)):
                B[s, s] += object_max[
                               s] / 100  # 为避免出现矩阵不可逆的情况，万一出现不可逆，则将斜对角上每个元素加一个较小的值（可以理解成，将与坐标轴平行的超平面在三个方向上都偏一点点，从而使得存在截距）
        else:
            a = np.dot(B1, np.ones((len(extreme_points),
                                    1)))  # B*a=C，则a=B的逆*C，这里C为（4，1）的所有元素为1的矩阵，整个解空间的超平面的方程即为 a * x1 + b * x2 + c * x3 + d * x4 = 1
...
def distance(S1, referpoint_set):
    ...
    for i in range(len(S1)):
        ...
        for j in range(len(referpoint_set)):
            d.append(1 - np.dot(S1[i][-1], referpoint_set[j]) / (np.linalg.norm(S1[i][-1]) * np.linalg.norm(
                referpoint_set[j])))  # 此处采用的是余弦距离，可参考：https://blog.csdn.net/lucky_kai/article/details/89514868
...
# 邻域搜索算法
def NS(generate):
    perturb_set = [perturb(i.copy()) for i in generate]
    return perturb_set
...
for run in range(1, max_run + 1):
    print('第' + str(run) + '次运行', '......')
    t3 = time.time()
    print('Configuration optimizing by NSGA-NS ......')
    '''初始种群生成'''
    temp_initials = pd.read_csv(
    ...
    while iter < max_iter and nochange_flag <= max_nochange:
        ...
        perturb_set = NS(childs)  # 邻域搜索
        adaption_perturb = adaption_calculate(perturb_set)  # 计算邻域解中各个个体的自适应值
        P_C = parents + childs + perturb_set  # 合并父代、子代和邻域解
        adaption_all = adaption_parents + adaption_childs + adaption_perturb  # 合并父代、子代和邻域解的自适应值
        iter += 1
    ...
    file.to_csv(os.path.join(result_dir, NSGANS_dir, 'edge_result_' + str(run) + '.csv'), index=False, header=False)
    t4 = time.time()
    run_time.append(t4 - t3)
    print('Configuration optimizing by NSGA-III is finished in ' + str(t4 - t3) + 's.')
pd.DataFrame(run_time).to_csv(os.path.join(result_dir, 'time', 'NSGANS.csv'), index=False, header=False)
