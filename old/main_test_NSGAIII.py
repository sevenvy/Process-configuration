...
if __name__ == '__main__': # 这里主要是用来避免多进程/线程时,反复重新导入数据
    ...
    while k1 < NG1 and k2 <= NG2:
        ...
        if num != N:
            S2 = copy.deepcopy(S1)
            S2 = nomalization(S2) # 目标函数解空间超平面的自适应标准化
        ...
        t401 = time.time()
        objects += [[k1, t401 - t40] + B[-1] for B in fronts[0]] # 保存每次迭代的最佳前沿，每行形式为[迭代次数，时间，优化目标1，优化目标2，优化目标3，优化目标4]
        ...
        #     t41 = time.time()
        #     with open(os.path.join('results', 'feature_solution_' + str(feature_solution_num), 'time_NSGAIII_400_' + str(test_num) + '.csv'), 'w', newline='') as f: # 保存成csv文件，可参考：https://www.cnblogs.com/xbhog/p/13141128.html
        #         writer = csv.writer(f)
        #         writer.writerow([t41-t1])
        #     with open(os.path.join('results', 'feature_solution_' + str(feature_solution_num), 'best_front_NSGAIII_400_' + str(test_num) + '.csv'), 'w', newline='') as f: # 保存成csv文件，可参考：https://www.cnblogs.com/xbhog/p/13141128.html
        #         writer = csv.writer(f)
    ...
    # 记录迭代600次或提前结束时的最佳前沿
    with open(os.path.join('results', 'feature_solution_' + str(feature_solution_num), 'best_front_NSGAIII_600_' + str(test_num) + '.csv'), 'w', newline='') as f: # 保存成csv文件，可参考：https://www.cnblogs.com/xbhog/p/13141128.html
        writer = csv.writer(f)
        writer.writerows(front_best)
    with open(os.path.join('results', 'feature_solution_' + str(feature_solution_num), 'time_NSGAIII_600_' + str(test_num) + '.csv'), 'w', newline='') as f: # 保存成csv文件，可参考：https://www.cnblogs.com/xbhog/p/13141128.html
        writer = csv.writer(f)
        writer.writerow([t5-t1])
    with open(os.path.join('results', 'feature_solution_' + str(feature_solution_num), 'object_NSGAIII_600_' + str(test_num) + '.csv'), 'w', newline='') as f: # 保存成csv文件，可参考：https://www.cnblogs.com/xbhog/p/13141128.html
        writer = csv.writer(f)
    ...
    with open(os.path.join('results', 'feature_solution_' + str(feature_solution_num), 'last_NSGAIII_600_' + str(test_num) + '.csv'), 'w', newline='') as f: # 保存成csv文件，可参考：https://www.cnblogs.com/xbhog/p/13141128.html
        writer = csv.writer(f)
        writer.writerows(last_iter)

    print('Configuration optimizing by NGSA-III is finished.')


# 在终端输入 python main_test_NSGAIII.py 运行本程序
