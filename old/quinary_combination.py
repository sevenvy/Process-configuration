...
if __name__=="__main__": # 如果是其他文件调用这个文件的函数，则只有其他部分会执行，这之后的部分不执行
    ...
    for line in Q:
        if J[line[0]-1][2] == 1 or J[line[0]-1][2] == 2 or J[line[0]-1][2] == 3 or J[line[0]-1][2] == 4: # 基准面1-4
            TJ1  = (500 - (M[line[2]-1][3] - 100000) / 900000 * 470) / 3 + 2 * (500 - (T[line[3]-1][2] - 1000) / 6500 * 470) / 3 # （30，500）秒范围内与机床和刀具价格成正比，机床和刀具价格的权重为1：2 + 三分之一上下范围内随机浮动
            TJ = (TJ1 + TJ1 / 3 * face_MT_random[face_MT.index([line[2], line[3]])]) / 3600
        elif J[line[0]-1][2] == 5 or J[line[0]-1][2] == 6: # 基准面5-6
            TJ1  = (500 - (M[line[2]-1][3] - 100000) / 900000 * 470) / 3 + 2 * (500 - (T[line[3]-1][2] - 1000) / 6500 * 470) / 3 # 同样的设备和刀具情况下为基准1-4的三分之二
            TJ = (TJ1 + TJ1 / 3 * face_MT_random[face_MT.index([line[2], line[3]])]) / 3600 * 2 / 3 
        elif J[line[0]-1][2] == 7: # 盲孔1-4
            if T[line[3]-1][0] == 1: # 铣削
                TJ1  = (100 - (M[line[2]-1][3] - 100000) / 900000 * 75) / 2 + (100 - (T[line[3]-1][2] - 1000) / 6500 * 75) / 2 #  (25：100）秒范围内与机床和刀具价格成正比，机床和刀具价格的权重为1：1+四分之一上下范围内随机浮动
            elif T[line[3]-1][0] == 2: # 钻削
                TJ1  = (24 - (M[line[2]-1][3] - 100000) / 900000 * 8) / 2 + (24 - (T[line[3]-1][2] - 1000) / 6500 * 8) / 2 # （16：24）秒范围内与机床和刀具价格成正比，机床和刀具价格的权重为1：1+四分之一上下范围内随机浮动
            else:
            ...
            if T[line[3]-1][0] == 1: # 铣削
                TJ1  = (600 - (M[line[2]-1][3] - 100000) / 900000 * 450) / 2 + (600 - (T[line[3]-1][2] - 1000) / 6500 * 450) / 2 #  （150：600）秒范围内与机床和刀具价格成正比，机床和刀具价格的权重为1：1+四分之一上下范围内随机浮动
            elif T[line[3]-1][0] == 2: # 钻削
                TJ1  = (240 - (M[line[2]-1][3] - 100000) / 900000 * 80) / 2 + (240 - (T[line[3]-1][2] - 1000) / 6500 * 80) / 2 #  （160：240）秒范围内与机床和刀具价格成正比，机床和刀具价格的权重为1：1+四分之一上下范围内随机浮动
            else:
            ...
            if T[line[3]-1][0] == 1: # 铣削
                TJ1  = (300 - (M[line[2]-1][3] - 100000) / 900000 * 225) / 2 + (300 - (T[line[3]-1][2] - 1000) / 6500 * 225) / 2 #  （75：300）秒范围内与机床和刀具价格成正比，机床和刀具价格的权重为1：1+四分之一上下范围内随机浮动
            elif T[line[3]-1][0] == 2: # 钻削
                TJ1  = (120 - (M[line[2]-1][3] - 100000) / 900000 * 40) / 2 + (120 - (T[line[3]-1][2] - 1000) / 6500 * 40) / 2 #  （80：120）秒范围内与机床和刀具价格成正比，机床和刀具价格的权重为1：1+四分之一上下范围内随机浮动
            else:
            ...
            if T[line[3]-1][0] == 1: # 铣削
                TJ1  = (900 - (M[line[2]-1][3] - 100000) / 900000 * 675) / 2 + (900 - (T[line[3]-1][2] - 1000) / 6500 * 675) / 2 #  （225：900）秒范围内与机床和刀具价格成正比，机床和刀具价格的权重为1：1+四分之一上下范围内随机浮动
            elif T[line[3]-1][0] == 2: # 钻削
                TJ1  = (360 - (M[line[2]-1][3] - 100000) / 900000 * 120) / 2 + (360 - (T[line[3]-1][2] - 1000) / 6500 * 120) / 2 #  （240：360）秒范围内与机床和刀具价格成正比，机床和刀具价格的权重为1：1+四分之一上下范围内随机浮动
            else:
                print('Error with the quinary combination data, position 5.')
                exit(0) 
            TJ = (TJ1 + TJ1 / 4 * hole_MT_random[hole_MT.index([line[2], line[3]])]) / 3600
        elif J[line[0]-1][2] == 12 or J[line[0]-1][2] == 13: # 内螺纹1-2
            TJ1 = (40 - (M[line[2]-1][3] - 100000) / 900000 * 20) / 2 + (40 - (T[line[3]-1][2] - 1000) / 6500 * 20) / 2 # （20：40）秒范围内与机床和刀具价格成正比，机床和刀具价格的权重为1：1+六分之一上下范围内随机浮动
            ...
            TJ1 = (90 - (M[line[2]-1][3] - 100000) / 900000 * 45) / 2 + (90 - (T[line[3]-1][2] - 1000) / 6500 * 45) / 2 # （45，90）秒范围内与机床和刀具价格成正比，机床和刀具价格的权重为1：1+六分之一上下范围内随机浮动
            ...
            TJ1 = (180 - (M[line[2]-1][3] - 100000) / 900000 * 90) / 2 + (180 - (T[line[3]-1][2] - 1000) / 6500 * 90) / 2 # （90，180）秒范围内与机床和刀具价格成正比，机床和刀具价格的权重为1：1+六分之一上下范围内随机浮动
            TJ = (TJ1 + TJ1 / 6 * step_MT_random[step_MT.index([line[2], line[3]])]) / 3600
        elif J[line[0]-1][2] == 20: # 3D打印体1
            TJ1 = 0.65 - (M[line[2]-1][3] - 100000) / 900000 * 0.525 # （0.125，0.65）小时范围内与机床价格成正比+四分之一上下范围内随机浮动
    ...
    for i in range(len(Q)):
        if T[Q[i][3]-1][0] != 7: # 非3D打印：（待机成本*4+上下四分之一范围内浮动）*加工时间
            CJ = (M[Q[i][2]-1][6] * 4 + M[Q[i][2]-1][6] * CJ_M[Q[i][2]-1]) * Q[i][5]
        elif T[Q[i][3]-1][0] == 7:
            if M[Q[i][2]-1][0] == [7]: # 纯3D打印设备的3D打印：（待机成本*8+上下四分之一范围内浮动）*加工时间
                CJ = (M[Q[i][2]-1][6] * 8 + 2 * M[Q[i][2]-1][6] * CJ_M[Q[i][2]-1]) * Q[i][5]
            else: # 混合加工设备的3D打印：（待机成本*6+上下四分之一范围内浮动）*加工时间
                CJ = (M[Q[i][2]-1][6] * 6 + 1.5 * M[Q[i][2]-1][6] * CJ_M[Q[i][2]-1]) * Q[i][5]
        else:
    ...
    i = 0
    for line in Q:
        if J[line[0]-1][2] == 1: # 基准面1
            AD = [1]
        elif J[line[0]-1][2] == 2 or J[line[0]-1][2] == 7 or J[line[0]-1][2] == 8: # 基准面2，盲孔1-4，通孔1-4
            AD = [5]
        elif J[line[0]-1][2] == 3: # 基准面3
            AD = [2]
        elif J[line[0]-1][2] == 4 or J[line[0]-1][2] == 9 or J[line[0]-1][2] == 10: # 基准面4，盲孔5-8，盲孔9-12
            AD = [6]
        elif J[line[0]-1][2] == 5 or J[line[0]-1][2] == 12: # 基准面5，内螺纹1
            AD = [3]
        elif J[line[0]-1][2] == 6 or J[line[0]-1][2] == 13: # 基准面6，内螺纹2
            AD = [4]
        elif J[line[0]-1][2] == 11 or J[line[0]-1][2] == 14 or J[line[0]-1][2] == 15 or J[line[0]-1][2] == 16 or J[line[0]-1][2] == 17 or J[line[0]-1][2] == 18 or J[line[0]-1][2] == 19: # 通孔5，内部台阶1-5，连续内部台阶1
            AD = [3, 4]
        elif J[line[0]-1][2] == 20: # 3D打印体1
            AD = [1, 2, 3, 4, 5, 6]
        else:
            print('Error with the quinary combination data, position 8.')
            exit(0)
        
        Q[i].append(AD) # 每行添加一个参数AD
        i += 1
    ...
    # 导出成csv文件
    with open(os.path.join(quinary_dir, quinary_dir + str(feature_solution_num) + '.csv'), 'w', newline='') as f: # 保存成csv文件，可参考：https://www.cnblogs.com/xbhog/p/13141128.html
        writer = csv.writer(f)
        writer.writerow(['quinary combination index', 'J Feature', 'P Process', 'M Machine', 'T Tool', 'F Fixture', 'TJ Processing duration', 'CJ Processing cost', 'AD Optional tool axis direction']) # 给每一列加上列名
        writer.writerows(Q)
